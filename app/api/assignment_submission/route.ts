...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      ...
      !assignmentSubmission.assignment_submission_id
    ) {
    ...
    for (const [questionId, fileKey] of Object.entries(files)) {
      ...
      try {
        ...
        console.error(
          `Failed to create submission for question ${questionId}:`,
          error
        );
      }
    }
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!assignmentId || !userId) {
      return NextResponse.json(
        { error: "Missing params - userId or assignmentId" },
        { status: 400 }
      );
    }

    const submission = await hasUserSubmittedAssignment(
      Number(assignmentId),
      userId
    );
